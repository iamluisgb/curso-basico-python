...
class Player:
    ...
    def place_ships(self):
        ...
        for ship in ships:
            while True:
                print(f"{self.name}, coloca tu {ship.name} de tamaóo {ship.size}.")
                start_row = int(input("Fila inicial: "))
                start_col = int(input("Columna inicial: "))
                direction = input("Direcciónn (H para horizontal, V para vertical): ").upper()
    ...
    def attack(self, opponent):
        while True:
            ...
            if 0 <= row < len(opponent.board) and 0 <= col < len(opponent.board[0]):
                if self.hits[row][col] in ('A', 'T'):
                    print("Ya has atacado esta posición. Intenta de nuevo.")
                    continue  # Vuelve a pedir una nueva posición
